...
-- Phiên bản: 1.1 (Performance Optimized - Anti-Lag ESP)
-- Tác giả: nguyen
...
-- TỐI ƯU HÀM FIX LAG (Dùng task.defer & chia nhỏ luồng quét để không giật lag)
FixLagBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        for i, obj in ipairs(descendants) do
            ...
            -- Cứ sau 100 vật thể thì tạm dừng 1 frame để tránh đứng màn hình
            if i % 100 == 0 then
                task.wait()
            end
        end
...
-- TỐI ƯU ESP ITEM (Giảm tần suất quét + Phân nhỏ công việc)
                ...
                if char and char:FindFirstChild("HumanoidRootPart") then
                    ...
                    for i, obj in ipairs(items) do
                        ...
                        if obj:IsA("BasePart") and LootNames[obj.Name] then
                            ...
                            -- Chỉ hiện ESP cho vật phẩm trong khoảng cách 1000m để giảm tải RAM
                            if dist <= 1000 then
                        ...
                        if i % 150 == 0 then task.wait() end -- Giảm tải CPU
                    end
                end
            end)
        end
        task.wait(1.5) -- Quét lại mỗi 1.5s thay vì liên tục
    end
end)
...
-- Khởi động UI
ShowUI()
print("Strawberry Hub v2.1 - Đã tối ưu hóa mượt mà!")
